...
# 	if dir:
# 		# dir *= SPEED * delta
# 		dir = dir.rotated(Vector3(1,0,0), rotation.x).normalized()
# 		dir = dir.rotated(Vector3(0,1,0), rotation.y).normalized()
		
# 	vel.x = dir.x * SPEED * delta
# 	vel.z = dir.z * SPEED * delta
...
# 		# if rot_y < -1: rot_y = -1
# 		# elif rot_y > 1: rot_y = 1
		
# 		transform.basis = Basis(Vector3(1,0,0), rot_x) #* Basis(Vector3(0,1,0), rot_y)
# 		# transform.basis = Basis(Vector3(0,1,0), rot_y)
# 		transform.basis = transform.basis * Basis(Vector3(0,1,0), rot_y)
